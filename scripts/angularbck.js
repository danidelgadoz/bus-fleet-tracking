...
app.controller('mainAppCtrl', function ($scope, $timeout, $interval, $mdSidenav, $log, myScriptGmap) {
  ...
  },60*1000); // <-- N // each N seconds the grid is refreshed, and 0.5 seg lather the markers in the map       
...
app.controller('menuListCtrl', function($scope, $mdDialog, $timeout, myAlert, myScriptGmap, $mdSidenav) {
    ...
    function DialogController($scope) {
        ...
        $scope.startTransbordox = function(unidad, tipo) {  
            ...
            for (var i = 0; i < unidadObj.length; i++){
                ...
                if(i == (unidadObj.length - 1))
                    myAlert.aviso("La unidad '" + unidad + "' no existe, ingrese una unidad valida.");            
            }      
        };
    };
      ...
      function Transbordo(obj, filtro){  
        ...
        if(filtro.id == 1){ // 'Toda la flota' ...
          ...
          myAlert.aviso("filtro no valido");
        }            
  ...
  $scope.openModalTrazarRuta = function() {
...
app.controller('trazarRutaCtrl', function($scope, myScriptGmap, myAlert) {
    ...
    for (var i = 0; i < unidadObj.length; i++){
        if($scope.unidad == unidadObj[i].data.name){
            ...
            if ( unidadToAprox.origen != "___" )
                ...
                myAlert.aviso("Ruta '" + unidadToAprox.origen + "' no es valida. Sí es valida, comuniquese con sistemas para su registro.");
            
            break;
        }

        if(i == (unidadObj.length - 1))
          myAlert.aviso("La unidad '" + $scope.unidad + "' no existe, ingrese una unidad valida.");
  ...
  function trazarRutaNow(i, origen_ini3, destino_ini3){                                              
    ...
    if( origenfound == true && destinofound == true )        
        ...
        myAlert.aviso("No es posible mostrar ruta(" + origen_ini3 + destino_ini3 + "). Informar incidencia a sistemas");    
  }             
});
...
app.controller('aproximacionesCtrl', function($scope, $timeout, myAlert) {
  ...
  $scope.showAproximaciones = function(){
    for (var i = 0; i < unidadObj.length; i++){
        if($scope.unidad == unidadObj[i].data.name){
            $scope.ruta = unidadObj[i].data.name + " (" +unidadObj[i].data.ruta + ")";
            $scope.recorrido = [];
            $scope.restante = [];
            $scope.total = []
  ...
  $scope.cleanFields = function clean(){
    $scope.ruta = null;
    $scope.buttonDisabled = true;
  ...
  function getAproximaciones(obj){
    ...
    $timeout(function (){    
      $scope.recorrido = { distancia: distMatrixGservice[1].distancia, tiempo: distMatrixGservice[1].tiempo };
      $scope.restante = { distancia: distMatrixGservice[0].distancia, tiempo: distMatrixGservice[0].tiempo };
      $scope.total = { distancia: distMatrixGservice[2].distancia, tiempo: distMatrixGservice[2].tiempo };
    }, 3500);
  }
  ...
  function pushArrayDMG(origen, destino, desc){      
      ...
      service.getDistanceMatrix({
          ...
          if (status !== google.maps.DistanceMatrixStatus.OK) {
              ...
              distMatrixGservice.push({
                  tiempo: tiempoGservice,
                  distancia: distanciaGservice,
                  descripcion: desc
              });
          }
      });
